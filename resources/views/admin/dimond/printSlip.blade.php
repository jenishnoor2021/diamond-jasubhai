  ...
  <script>
    const diamondData = [
      @foreach($data as $diamond) {
        ...
        party_code: '{{ $diamond->parties->party_code }}',
        date: '{{ \Carbon\Carbon::parse($diamond->created_at)->format("d/m") }}'
      },
      @endforeach
    ];
    ...
    function generateBarcode(value, index) {
      ...
      info.innerHTML = `<b>${diamondData[index].party_code}-${diamondData[index].date}&nbsp;|&nbsp;${diamondData[index].name}&nbsp;|&nbsp;${diamondData[index].weight}</b>`;

      const label = document.createElement('p');
      label.style.fontSize = '8px';
      label.style.marginTop = '-20px';
      label.style.marginLeft = '50px';
      label.innerText = 'HR Diamond';
